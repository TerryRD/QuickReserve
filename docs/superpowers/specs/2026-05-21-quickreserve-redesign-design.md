...
> ⚠️ **文件維護原則**：本 spec 是 Single Source of Truth。每次新增 / 修改功能後必須回來更新對應的 FR，並於上方 metadata 標註「最後更新」日期。同步更新 [`docs/user-guides/`](../../user-guides/) 對應的角色手冊。
...
### 4.8 學員管理（UX audit 後新增）
- **FR-070**: 教練後台「學員」頁列出有跟自己（或租戶內任何成員）預約過的所有客戶
- **FR-071**: 每位學員顯示總預約數、待確認數、已完成數、最近預約日期
- **FR-072**: 學員資料隔離仍嚴格（透過 `tenant_customers` bridge，跨租戶不可見）

### 4.9 重複規則管理（UX audit 後新增）
- **FR-073**: 教練可在「行事曆 → 管理重複規則」（`/calendar/rules`）查看所有既有規則
- **FR-074**: 每筆規則顯示人類可讀描述（例：「每週一三五 19:00–21:00 共 12 次」）+ 未來剩餘時段數
- **FR-075**: 可切換規則「啟用 / 停用」狀態（停用後 cron 不再為其產生新 slot）
- **FR-076**: 可刪除規則；刪除時可勾選「同時清除未來尚未被預約的時段」
  - 刪除規則本身：透過 `ON DELETE SET NULL` 既有 slot 的 `recurring_rule_id` 變 null
  - 已被預約的 slot（`pending` / `booked`）**不會**被刪，學員預約紀錄保留

### 4.10 多成員行事曆檢視（UX audit 後新增）
- **FR-077**: Owner 可在 `/calendar?members=...` 同時檢視多個成員的時段
- **FR-078**: 預設顯示「全部成員」，可用 checkbox 多選篩選器調整
- **FR-079**: 多成員視圖下，每個時段顯示成員標籤；其他成員的時段為虛線邊框且唯讀
- **FR-080**: 點擊時段彈出 detail（服務、時間、成員、狀態、學員）+ 刪除按鈕（限自己的可預約時段）

### 4.11 平台管理員 drill-in（UX audit 後新增）
- **FR-081**: 平台管理員可點擊租戶列表進入單一租戶的唯讀詳情頁 `/platform/tenants/[id]`
...
| 2026-05-25 | service_packages schema + CRUD UI + 軟刪除 | FR-125 | `343bb98`, `c70a376` |
| 2026-05-25 | customer_purchases schema + 學員申請 + 教練審核 + 通知 | FR-126 | `4a5d6e9`, `ce27aeb`, `c362905` |
| 2026-05-25 | 預約強制經過 purchase + book_with_purchase RPC | FR-127 | `e6823f8`, `23fad28`, `579e713`, `b3ad9e8` |
