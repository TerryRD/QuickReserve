...
export const createRecurringRuleAction = actionClient
  ...
  .action(async ({ parsedInput }): Promise<CreateResult> => {
    ...
    const { data: svc } = await supabase
      .from('services')
      .select('id, tenant_id')
      .eq('id', parsedInput.serviceId)
      .maybeSingle()
    if (!svc || svc.tenant_id !== session.tenantId)
      throw new AppError('SERVICE_NOT_FOUND', '服務不存在')

    // 2. Compute occurrences in materialization window (start_date → +90 days, clipped to end_until)
    const startDate = new Date(`${parsedInput.startDate}T00:00:00+08:00`)
    ...
    // 3. Pre-query existing non-cancelled slots for this member overlapping our range
    ...
    // 4. Split occurrences into to-insert vs conflicts
