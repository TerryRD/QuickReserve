...
3. ~~**slot popover live conflict-detection inline**~~ — ✅ 2026-05-29 ship。`recurring-actions.ts` 抽出 `computeRulePreview` helper 供 create + preview 共用,加 `previewRecurringRuleAction`(無 DB writes,looser schema)。Dialog 用 300ms debounced `useEffect` 呼叫 preview,顯示「共 N 個時段 · 其中 K 個衝突 · M 個落在作息外」。Lighter schema 不會在 user 輸入未完整時噴 toast。
4. ~~**`/<slug>/packages` 第 3 種付款狀態 `部份付` + receipt note**~~ — ✅ 2026-05-29 ship。Migration `20260529130000_customer_purchases_partial_paid_and_note.sql` 動態 lookup 舊 CHECK constraint name → drop → 加新 CHECK 含 `'partial_paid'` + 加 `receipt_note text` nullable。Customer 端 purchase-request-form 加狀態 + 條件 textarea(只在非 awaiting_payment 顯示);action schema 加 enum + receiptNote;coach pending 端 PAID_LABEL/COLOR map + RECEIPT block 顯示 note。
5. ~~**`/notifications` persistent read state**~~ — ✅ 2026-05-29 ship。Migration `20260529120000_notification_log_read_at.sql` 加 `read_at timestamptz` + RLS UPDATE policy(self only)+ partial unread index;server actions `markNotificationReadAction` / `markAllNotificationsReadAction`;UI 改用 `read_at IS NULL` + per-row click + 「全部標為已讀(N)」按鈕。
...
- ⚠️ **HIBP leaked-password 鎖在 Pro Plan**(2026-05-29 試 Management API PATCH 回 `HTTP 402`)— advisor 那條 WARN 在 Free Plan 永遠關不掉。**App-layer 替代方案已 ship**:`src/lib/password-strength.ts` 包 `@zxcvbn-ts/core` + score≥2 threshold + 接 signup Zod refine,擋常見字典詞 / 純數字 / 鍵盤序列 / 短弱組合。6 unit test。memory `project_supabase_free_plan_limits.md` 已記。
